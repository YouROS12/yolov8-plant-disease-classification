...
\end{itemize}
ResNet50 and EfficientNet-B0 were used as frozen feature extractors with their original ImageNet-pretrained \cite{russakovsky2015imagenet} weights. YOLOv8m was trained from scratch on the PlantWildV2 training split. In all cases, features were extracted once and stored as a static bank.
...
In terms of model performance and applicability, we found that tree-based models like RandomForest and XGBoost perform well but essentially reach the plateau of the training data while having a significant gap between training and test performance \cite{sahin2020assessing}. By contrast, the SVC model had the best performance with a smaller gap, suggesting that the feature space constructed by YOLOv8+PCA aligns well with the max-margin principle. The two-stage model provides two benefits: high performance on the test set with good generalization, and a separate model design that allows for rapid retraining and efficient inference, which can be useful for scenarios with strict memory and processing constraints.
